...
REMOTE_LOG_BACKEND="${REMOTE_LOG_BACKEND:-<remote-log-backend>}"
REMOTE_LOG_ENV_SELECTOR="${REMOTE_LOG_ENV_SELECTOR:-<remote-log-environment-selector>}"

if [[ "$REMOTE_LOG_BACKEND" == "<remote-log-backend>" ]]; then
  die "Bootstrap required: replace REMOTE_LOG_BACKEND placeholder"
fi
if [[ "$REMOTE_LOG_ENV_SELECTOR" == "<remote-log-environment-selector>" ]]; then
  die "Bootstrap required: replace REMOTE_LOG_ENV_SELECTOR placeholder"
fi
